    ...
    flights.forEach(flight => {
        const row = `
            <tr>
                ...
                <td>
                    <button onclick="modifyFlight(${flight.flight_id})">수정</button>
...
// 항공편 수정 페이지로 이동
function modifyFlight(id) {
    if (confirm('이 항공편을 수정하시겠습니까?')) {
        // 항공편 상세 정보 페이지로 이동, id를 쿼리 파라미터로 전달
        window.location.href = `flight-modify.html?id=${id}`;
    }
}
